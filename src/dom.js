...
(function (window, document, undefined) {
  ...
  function _isLiteralObject(object) {
    return object && _typeof(object) === "object" && Object.getPrototypeOf(object) === Object.getPrototypeOf({});
  }
  ...
  function _isIterable(object) {
    ...
    var r = _isLiteralObject(object) || _isArray(object) || _typeof(object) === 'object' && object !== null && object['length'] !== undefined;
    return r;
  }
  ...
  function _each(object, callback) {
    if (_isArray(object) || _typeof(object) === 'object' && object['length'] !== undefined) {
      for (var i = 0, l = object.length; i < l; i++) {
  ...
  var cssNameProperty = function cssNameProperty(prop) {
    return prop;
  };
  ...
  if (ie && ie < 9) {
    cssNameProperty = function cssNameProperty(prop) {
  ...
  Dom.isElement = function (element) {
    if ((typeof HTMLElement === "undefined" ? "undefined" : _typeof(HTMLElement)) === 'object') {
      return _instanceof(element, HTMLElement);
    }

    return element && _typeof(element) === 'object' && element.nodeType === 1 && typeof element.nodeName === 'string';
  };
  ...
  Dom.isNode = function (node) {
    if ((typeof Node === "undefined" ? "undefined" : _typeof(Node)) === 'object') {
      return _instanceof(node, Node);
    }

    return node && _typeof(node) === 'object' && typeof node.nodeType === 'number' && typeof node.nodeName === 'string';
  };
  ...
  Dom.dispatch = function (element, type, options) {
    ...
    for (var name in eventObjects) {
      if (eventObjects[name].test(type)) {
        eventClass = name;
        break;
      }
    }
    ...
    if (document.createEvent) {
      ...
      if (eventClass == 'MouseEvents') {
        event.initMouseEvent(o.type, o.bubbles, o.cancelable, o.view, o.detail, o.screenX, o.screenY, o.clientX, o.clientY, o.ctrlKey, o.altKey, o.shiftKey, o.metaKey, 0, null);
      } else {
  ...
  Dom.attribute = function (element, attribute) {
    ...
    if (typeof attribute === "string") {
      ...
      if (attribute === 'class' && element['className'] !== undefined) {
        //class?
        result = element.className;
      } else if (attribute === 'for' && element['htmlFor'] !== undefined) {
        //for?
        result = element.htmlFor;
      } else if (attribute === 'value' && element['value'] !== undefined) {
        //value?
  ...
  /**
   * Caller function
   * In ES5 strict mode, arguments.callee was deprecated so this is the replacement. 
   * @param {function} fn 
   */
  function _caller(fn) {
    return function () {
      return fn.bind(null, _caller(fn)).apply(this, arguments);
    };
  }
  
  //on ready
  if (addListener === 'attachEvent') {
    //shitty browsers
    document[addListener]('onreadystatechange', _caller(function (callee, e) {
      if (document.readyState === 'complete') {
        document[removeListener]('onreadystatechange', callee);

        _onDOMReady(e);
      }
    }));
  } else {
    //ecma compatible browsers
    document[addListener]('DOMContentLoaded', _caller(function (callee, e) {
      document[removeListener]('DOMContentLoaded', callee, false);
